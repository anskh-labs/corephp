...
namespace Corephp\Component\Mail;
 ...
 * SMTPSender
 * -----------
 * SMTPSender modified from https://github.com/arkanis/smtp_send
 *
 * @author Khaerul Anas <anasikova@gmail.com>
 * @since v1.0.0
 * @package Corephp\Component\Mail
 */
class SMTPSender
{
    /**
    ...
    public function initialize(array $config): void
    {
        if (isset($config['smtp_server'])) {
            $this->setSmtpServer($config['smtp_server']);
        }
        if (isset($config['smtp_port'])) {
            $this->setSmtpPort($config['smtp_port']);
        }
        if (isset($config['options'])) {
            $this->setSmtpOptions($config['options']);
        }
    }    
    /**
     * setSmtpServer
     ...
     * @return void
     */
    public function setSmtpOptions(array $options ): void
    {
        $this->options = $options;        
    }
    /**
     * `send` sends mails directly via SMTP. It uses `STARTTLS` when the server
     ...
     *     End of message.
     *     EOD;
     *     send('sender@dep1.example.com', 'receiver@dep2.example.com', $message);
     * 
     * 
     ...
     *   constructing it.
     * 
     *   https://tools.ietf.org/html/rfc5322#appendix-A
     * 
     * Return value
     * ------------
     * 
     ...
     *     End of message.
     *     EOD;
     *     send('sender@example.com', 'receiver@example.com', $message);
     * 
     * * * *
     * 
     ...
     * -------
     * 
     * `send()` function © 2018 Stephan Soller  
     ...
     */
    public function send(string $from, string|array $to, string $message): bool
    {
        // Set default values
        ...
        $con = fsockopen($this->smtp_server, $this->smtp_port, $errno, $errstr, $options['timeout']);
        if ($con === false)
            return false;
